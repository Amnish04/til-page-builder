    ...
    def run(self):
        ...
        if os.path.isdir(input_path):
            ...
            for file in files_in_directory:
                files_to_be_generated.append(self.html_builder.generate_html_for_file(file))

            self.html_builder.generate_files(files_to_be_generated)

        elif os.path.isfile(input_path):
            # Check if a text file is supplied
            if has_txt_extension(input_path) or has_md_extension(input_path):
                self.html_builder.generate_files([self.html_builder.generate_html_for_file(input_path)])
            else:
